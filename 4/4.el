...
(progn
  ...
  (defun contain-range (range-1 range-2)
    (let
        (
         (a (car range-1))
         (b (car (last range-1)))
      ...
      (or (eq (>= a c) (<= b d)) (eq (<= a c) (>= b d)))
      )
    )

  (defun contain-range-helper (list)
    (if (contain-range (car list) (car (last list))) 1 0))

  (defun get-contain-bool (list)
    (mapcar #'contain-range-helper list))
